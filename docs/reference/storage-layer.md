...
2. `scripts/storageSurface.ts` lets a reviewer prove that a change did not
   drop, duplicate, or silently rewrite a method (see [Verifying these
   documents](./verifying-these-docs.md)). It builds its baseline from a git
   ref - `origin/main` by default - rather than a committed snapshot: the
   snapshot it used to read was generated by the same commit that finished the
   decomposition, so it could not have caught a method lost during it.
