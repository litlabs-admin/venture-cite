...
export async function setupStripeProducts() {
  ...
  if (!productNames.includes('Free')) {
    ...
    const freeProduct = await stripe.products.create({
      ...
      metadata: {
        tier: 'free',
        features: '1 brand profile,5 AI-generated articles/month,Auto-humanization included,Basic GEO rankings,Community support',
      },
    });
  ...
  if (!productNames.includes('Pro')) {
    ...
    const proProduct = await stripe.products.create({
      ...
      metadata: {
        tier: 'pro',
        popular: 'true',
        features: '5 brand profiles,40 AI-generated articles/month,Auto-humanization & AI detection,Full GEO rankings & analytics,AI Intelligence dashboard,Publication Intelligence,Priority support',
      },
    });
    await stripe.prices.create({
  ...
  if (!productNames.includes('Enterprise')) {
    ...
    const enterpriseProduct = await stripe.products.create({
      ...
      metadata: {
        tier: 'enterprise',
        features: 'Everything in Pro,Unlimited brand profiles,200 AI-generated articles/month,GEO AI Agent automation,AI Traffic Analytics,Custom integrations,Dedicated account manager,SSO & advanced security',
      },
    });
    await stripe.prices.create({
