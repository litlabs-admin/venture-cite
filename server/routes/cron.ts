...
// extracted as part of the B7 service-layer split. The Orchestrator class
// and the STEP_CAPS_MS budget table live in server/services/cronOrchestrator
// - only the sequencing of `orch.run("step-name", ...)` calls below stays
// here, because which steps run, in what order, gated on which day-of-week
// or env var, is orchestration policy specific to this route.
//
// tests/unit/schedulerOrchestratorParity.test.ts drives this route's real
// HTTP handler (with its dependencies stubbed) to observe which steps
// actually run, and checks that set against the in-process scheduler's job
// registry and against STEP_CAPS_MS - both imported as real data, not
