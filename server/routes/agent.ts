...
export function setupAgentRoutes(app: Express): void {
      ...
      try {
        ...
        try {
          ...
          if (err instanceof AgentTaskExecutionError) {
            ...
            captureAndFlush(err, { tags: { source: "agent.ts:208" } });
  ...
  app.get(
    "/api/outreach-campaigns/:brandId",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
  ...
  app.get(
    "/api/outreach-campaigns/stats/:brandId",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
        const { brandId } = req.params;
  ...
  app.get(
    "/api/outreach-campaigns/detail/:id",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
  ...
  app.get(
    "/api/automation-executions/:ruleId",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
  ...
  app.get(
    "/api/publication-targets/:brandId",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
        const { brandId } = req.params;
  ...
  app.post(
    "/api/publication-targets/discover",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
        ...
        if (!brandId || !industry) {
          return res
            .status(400)
            .json({ success: false, error: "brandId and industry are required" });
        }
        await requireBrand(brandId, user.id);
  ...
  app.post(
    "/api/publication-targets/:id/find-contacts",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
  ...
  app.get(
    "/api/publication-targets/detail/:id",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
  ...
  app.get(
    "/api/outreach-emails/stats/:brandId",
    asyncHandler(async (req, res) => {
      try {
        const user = requireUser(req);
