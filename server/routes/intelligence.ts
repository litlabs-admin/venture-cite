        ...
        if (slackWebhookUrl !== undefined && slackWebhookUrl !== null && slackWebhookUrl !== "") {
          if (!isValidSlackWebhookUrl(slackWebhookUrl)) {
            return res.status(400).json({
              success: false,
              error:
                "Invalid Slack webhook URL — must be of the form https://hooks.slack.com/services/T.../B.../...",
            });
          }
        }
  ...
  app.patch(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        captureAndFlush(error, { tags: { source: "intelligence.ts:784" } });
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        // Two-layer defence: (1) strict canonical URL shape, in case a legacy
        // row predates the validator at the write paths; (2) assertSafeUrl
        // resolves DNS and rejects private/loopback/metadata IPs at fetch
        // time, blocking DNS-rebinding against an attacker-controlled host
        // that happens to match the regex.
        if (setting.slackEnabled === 1 && setting.slackWebhookUrl) {
          try {
            if (!isValidSlackWebhookUrl(setting.slackWebhookUrl)) {
              errors.push(
                "Invalid Slack webhook URL — expected https://hooks.slack.com/services/...",
              );
            } else {
              const safe = await assertSafeUrl(setting.slackWebhookUrl);
        ...
        captureAndFlush(error, { tags: { source: "intelligence.ts:891" } });
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        captureAndFlush(error, { tags: { source: "intelligence.ts:910" } });
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        captureAndFlush(error, { tags: { source: "intelligence.ts:955" } });
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        captureAndFlush(error, { tags: { source: "intelligence.ts:998" } });
