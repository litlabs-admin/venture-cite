...
export interface IStorage {
  ...
  // (case-insensitive). Used by the variant-learning loop and the UI.
  addBrandNameVariation(brandId: string, variation: string): Promise<boolean>;
  addCompetitorNameVariation(competitorId: string, variation: string): Promise<boolean>;
  ...
  getLatestBrandVisibility(brandId: string): Promise<{
  ...
  getHallucinationStats(brandId: string): Promise<{
    total: number;
    resolved: number;
...
function databaseStorageObject(): Omit<
  IStorage,
  keyof typeof chatbotStorage | keyof typeof competitorsStorage | keyof typeof identityStorage
> {
