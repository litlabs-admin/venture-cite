...
import { claimStripeWebhookEvent, maintainStripeWebhookClaim } from "./lib/stripeWebhookClaim";
...
export class WebhookHandlers {
  static async processWebhook(payload: Buffer, signature: string): Promise<void> {
    ...
    if (claim.kind === "complete") {
      logger.info({ eventId: event.id, type: event.type }, "stripe webhook: the event is complete");
      return;
    }
    if (claim.kind === "busy") {
      throw new Error(`Stripe webhook event ${event.id} has an active processing claim`);
    }

    const lease = maintainStripeWebhookClaim(event.id, claim.token);
    ...
    try {
      ...
      const completed = await lease.finish();
      if (!completed) {
        throw new Error(`Stripe webhook event ${event.id} lost its processing claim`);
      }
    } catch (error) {
      await lease.stop();
      throw error;
