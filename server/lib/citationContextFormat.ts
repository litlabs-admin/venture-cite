...
// The failed-call classifier lives in `shared/citationFailure.ts` because a
// client screen needs it too, and `client/src/**` cannot import `server/**`.
// Re-exported here so server callers keep importing it from the module that
// owns every other citation_context format rule, and so no second definition
// can drift from the first.
export {
  CHECK_FAILED_PREFIX,
  CHECK_FAILED_LIKE_PATTERN,
  isFailedCheck,
} from "@shared/citationFailure";
