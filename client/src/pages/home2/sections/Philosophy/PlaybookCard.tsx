...
export function PlaybookCard({
  ...
  return (
    ...
    >
      <span className="text-[20px] sm:text-[24px] lg:text-[28px] font-semibold tabular-nums tracking-tight text-tk-accent/30 group-hover:text-tk-accent transition-colors duration-300 mb-3 sm:mb-4">
        {number}
      </span>
      <h3 className="text-[15px] sm:text-[16px] font-semibold text-tk-primary tracking-[-0.02em] leading-snug mb-2 sm:mb-3">
        {title}
      </h3>
      <p className="text-[13px] sm:text-[14px] text-tk-secondary leading-relaxed">{body}</p>
      <div className="mt-4 sm:mt-5 mb-6 sm:mb-7 min-h-[140px] flex w-full items-center">
        {children}
      </div>
      <span className="mt-auto text-[12px] sm:text-[13px] font-medium text-tk-accent flex items-center gap-1.5">
        {ctaLabel}
