    ...
    <div className="flex items-center gap-2.5" style={{ height: 38 }}>
      <span
        className={`w-[26px] flex-shrink-0 text-[9px] font-medium uppercase tracking-wider ${
          message.role === "You" ? "text-tk-text-muted" : "text-tk-accent"
        }`}
      >
        {message.role}
      </span>
      <span
        className={`min-w-0 flex-1 truncate text-[12.5px] leading-snug ${
          message.role === "You" ? "text-tk-secondary" : "text-tk-primary"
        }`}
      >
        {message.text}
      </span>
      {message.tag && (
        <span
          className={`ml-auto flex flex-shrink-0 items-center gap-1.5 text-[10px] font-medium ${
            message.tag.pulsing ? "text-tk-text-muted" : "text-tk-accent"
          }`}
        >
          {message.tag.pulsing ? (
