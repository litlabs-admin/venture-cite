  ...
  return (
    <div className="flex flex-col h-full p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] font-medium text-tk-secondary">Crawlers</p>
      </div>
      <div className="relative">
        ...
        <div className="flex justify-between mt-1">
          <span className="text-[8px] font-mono text-tk-tertiary">24h ago</span>
          <span className="text-[8px] font-mono text-tk-text-muted">Now</span>
        </div>
      </div>
      ...
      <div className="mt-3 mb-3">
        ...
        <span className="text-[10px] text-tk-text-muted mt-0.5 block">crawls this week</span>
      </div>
      ...
      <div className="space-y-1 mt-auto">
        {CRAWLER_ROWS.map((row) => (
          ...
          >
            ...
            />
            <span className="w-[80px] text-[11px] truncate transition-colors text-tk-secondary">
              {row.name}
            </span>
            <div className="flex-1 h-[3px] bg-tk-muted overflow-hidden rounded-sm">
              <div
                className="h-full"
                style={{ width: `${row.widthPct}%`, backgroundColor: row.color }}
              />
            </div>
            <span className="text-[10px] font-mono tabular-nums text-tk-text-muted w-8 text-right">
              {row.count}
            </span>
          </div>
        ))}
