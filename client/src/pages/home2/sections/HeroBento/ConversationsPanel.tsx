  ...
  return (
    <div className="flex flex-col h-full p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] font-medium text-tk-secondary">Conversations</p>
      </div>
      ...
      <div className="mb-3">
        ...
        <span className="text-[10px] text-tk-text-muted mt-0.5 block">
      ...
      <div className="flex-1 min-h-0 -mx-2 mt-1">
        <div className="flex items-center gap-1.5 mb-2 px-2">
          <span className="relative flex w-1.5 h-1.5">
            <span className="absolute inset-0 rounded-full bg-tk-accent opacity-75 animate-ping" />
            <span className="relative w-1.5 h-1.5 rounded-full bg-tk-accent" />
          </span>
          <span className="text-[10px] font-medium text-tk-accent">Live</span>
        </div>
          ...
          {CONVERSATION_ROWS.map((row) => (
            ...
            >
              <div className="flex items-center gap-2 px-2 py-1.5">
                ...
                />
                <span className="flex-1 min-w-0 text-[11px] text-tk-secondary truncate group-hover:text-tk-primary transition-colors">
                  {row.query}
                </span>
                <span className="shrink-0 text-[10px] font-mono tabular-nums w-[88px] text-right truncate text-tk-tertiary">
                  {row.resultPath}
                </span>
                <span className="shrink-0 text-[10px] font-mono tabular-nums w-12 text-right text-tk-text-muted">
                  {row.timeAgo}
                </span>
              </div>
            </div>
