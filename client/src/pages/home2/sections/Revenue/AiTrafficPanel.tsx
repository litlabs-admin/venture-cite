...
const RECENT_PAGES = [
  { alt: "ChatGPT", src: "/trakkr/images/ai-logos/chatgpt.svg", path: "/pricing", time: "2m" },
  {
    alt: "Perplexity",
  ...
  { alt: "Claude", src: "/trakkr/images/ai-logos/claude.png", path: "/features", time: "14m" },
  { alt: "ChatGPT", src: "/trakkr/images/ai-logos/chatgpt.svg", path: "/demo", time: "21m" },
  { alt: "Gemini", src: "/trakkr/images/ai-logos/gemini.svg", path: "/docs/api", time: "28m" },
  { alt: "Perplexity", src: "/trakkr/images/ai-logos/perplexity.svg", path: "/about", time: "34m" },
];
    ...
    <div className="p-4 sm:p-5 lg:p-6 flex flex-col h-full">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-tk-accent" />
          <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-tk-text-muted">
            AI Traffic
          </span>
        </div>
        <span className="text-[10px] text-tk-text-muted">Last 48h</span>
      </div>
      ...
      >
        <div className="flex items-baseline gap-2">
          <span
            className="text-[28px] font-semibold tracking-tight text-tk-primary tabular-nums leading-none transition-colors duration-300"
            style={{ filter: "brightness(1)" }}
          >
            2,847
          </span>
          <span className="text-[12px] text-tk-text-muted">sessions</span>
      ...
      >
        <div className="relative">
          ...
          <div className="flex items-center justify-between mt-1.5">
            <span className="text-[9px] text-tk-text-muted font-mono tabular-nums">48h ago</span>
            <span className="text-[9px] text-tk-secondary font-mono">Now</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="space-y-0 -mx-2 overflow-hidden">
          {RECENT_PAGES.map((page, i) => (
            <div
              key={i}
              className="flex items-center gap-2.5 py-1.5 px-2 rounded hover:bg-tk-muted group cursor-default transition-all duration-300"
            >
              <img
                src={page.src}
                alt={page.alt}
              ...
              />
              <span className="flex-1 text-[11px] text-tk-secondary font-mono truncate group-hover:text-tk-primary transition-colors">
                {page.path}
              </span>
              <span className="text-[10px] tabular-nums text-tk-text-muted flex-shrink-0 transition-all duration-300">
                {page.time}
              </span>
            </div>
          ))}
