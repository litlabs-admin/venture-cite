  ...
  return (
    <button
      ...
      className={`h-9 px-4 text-[13px] font-medium transition-all duration-250 rounded flex items-center gap-1.5 ${
        isOpen
          ? "text-tk-primary bg-tk-muted"
          : "text-tk-secondary hover:text-tk-primary hover:bg-tk-muted"
      }`}
    >
      {label}
      <ChevronDownIcon
  ...
  return (
    <div
      ...
      className={`
        ...
        bg-tk-surface rounded border border-tk-default shadow-[0_8px_28px_-12px_rgba(15,23,42,0.12)]
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col min-w-0 pt-1">
        <span className="text-[13px] font-medium text-tk-primary leading-tight tracking-tight transition-colors group-hover:text-tk-accent-hover">
          {item.name}
        </span>
        <span
...
export function MegaMenuLinkRow({
  ...
  const className = `
    ...
    text-[12.5px] font-medium text-tk-secondary tracking-tight
    transition-all
    hover:text-tk-accent-hover hover:bg-tk-accent-subtle/40
    ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1"}
  `;
  const style = {
  ...
  const content = (
    <>
      ...
      <ArrowRightIcon
        size={11}
        className="text-tk-accent opacity-0 -translate-x-1 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0"
      />
    </>
  );
  if (item.external) {
