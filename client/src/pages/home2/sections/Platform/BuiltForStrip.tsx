...
export function BuiltForStrip({ isVisible }: { isVisible: boolean }) {
  return (
    <div
      className="border-b border-tk-default bg-tk-surface px-4 sm:px-6 lg:px-8 py-3 sm:py-3.5 transition-all duration-500"
          ...
          {items.map((item, i) => (
            <span key={item.label} className="flex items-center gap-3 sm:gap-5">
              {item.href ? (
                <a
                  href={item.href}
                  className="group inline-flex items-center gap-1 text-[12px] sm:text-[13px] font-medium text-tk-secondary hover:text-tk-accent transition-colors duration-150"
                >
                  {item.label}
                  <ArrowRightIcon
                    size={11}
                    className="text-tk-text-muted/60 group-hover:text-tk-accent group-hover:translate-x-0.5 transition-all duration-150"
                  />
                </a>
              ) : (
                <span className="text-[12px] sm:text-[13px] font-medium text-tk-secondary">
                  {item.label}
                </span>
              )}
              {i < items.length - 1 && <span className="w-1 h-1 rounded-full bg-tk-default" />}
            </span>
          ))}
        </div>
      </div>
