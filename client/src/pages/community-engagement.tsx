...
export default function CommunityEngagement() {
  ...
  const [activeTab, setActiveTab] = useState<"discover" | "drafts" | "posted">("discover");
  const [editingDraft, setEditingDraft] = useState<CommunityPost | null>(null);
  const [draftEditForm, setDraftEditForm] = useState({ title: "", content: "" });
  ...
  const discoverMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Communities discovered",
        description: `Found ${(data.data || []).length} relevant groups`,
      });
    },
    onError: () => {
      toast({
        title: "Discovery failed",
        description: "Could not discover communities. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  // Marks a draft as posted AND switches the user to the Posted tab so the
  ...
  const markPostedMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success === false) {
        toast({
          title: "Failed to mark as posted",
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingDraft ? `${editingDraft.platform} · ${editingDraft.groupName}` : ""}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setDraftEditForm({ ...draftEditForm, title: e.target.value })}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!draftEditForm.content.trim() || updatePostMutation.isPending}
