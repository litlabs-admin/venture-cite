# AGENTS.md — Operating context for AI-paired development

This file is loaded by Codex on every session start. Keep it tight; link out for depth.
...
VentureCite — pre-launch GEO SaaS. Single-tenant rooted at `users`; brands hang off users; everything else hangs off brands. Single-instance deployment for now (multi-instance items in [`AUDIT.md`](AUDIT.md) are deferred — see plan in `~/.Codex/plans/`).
...
| What's the remediation order? | `~/.Codex/plans/how-should-we-go-noble-reddy.md` |
