...
Find what a change can break outside the diff before it ships. Use for "blast radius of X" or a small diff that needs review.
...
6. For a big or wide change, ask up to two Codex collaboration agents to check separate risk areas. Use read-only scopes.
...
Write it through `unslop`. Cite real code. Do not include secrets or user data.

Use a local, preview, or isolated test path for proof. Do not access production data, send email, charge a payment method, or publish content.
