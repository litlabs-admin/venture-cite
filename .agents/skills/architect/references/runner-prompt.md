...
The parent gives this prompt to a Codex collaboration agent with the task, grounding evidence, and a read-only scope.

Produce one independent design. Read the `architect` skill. Return a type sketch, function signatures, module map, and a rationale that follows [`rationale-template.md`](rationale-template.md).
...
Keep the task read-only unless the parent grants write access. Do not read `.env`, use secrets, access live user data, or send external messages. Use Luna low for inventory. Use Terra medium only for design synthesis. Do not select unavailable models.
