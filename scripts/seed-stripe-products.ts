...
async function seedProducts() {
  ...
  const proProduct = await stripe.products.create({
    ...
    metadata: {
      tier: 'pro',
      popular: 'true',
      features: 'Unlimited brand profiles,50 AI-generated articles/month,Full GEO rankings & analytics,AI Intelligence dashboard,Publication Intelligence,Priority support'
    }
  });
  ...
  const enterpriseProduct = await stripe.products.create({
    ...
    metadata: {
      tier: 'enterprise',
      popular: 'false',
      features: 'Everything in Pro,Unlimited articles,GEO AI Agent automation,AI Traffic Analytics,Custom integrations,Dedicated account manager,SSO & advanced security'
    }
  });
