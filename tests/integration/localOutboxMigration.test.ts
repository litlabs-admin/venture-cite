...
describeIfLocal("local transactional outbox migration", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(`CREATE TABLE ${quotedSchema}.llm_jobs (
  ...
  it("enqueues an OpenAI LLM job through the restricted request role", async () => {
    await applyLlmJobOutboxMigration();
    await pool.query(
      `INSERT INTO ${quotedSchema}.users (id) VALUES ('llm-user') ON CONFLICT DO NOTHING`,
    );
    await pool.query(
      `INSERT INTO ${quotedSchema}.brands (id, user_id, deleted_at)
       VALUES ('llm-brand', 'llm-user', NULL) ON CONFLICT DO NOTHING`,
    );
    await pool.query(
      `INSERT INTO ${quotedSchema}.llm_jobs
       (id, kind, status, payload, provider_request, user_id, brand_id)
       VALUES (
         'llm-job-1', 'keyword_discovery', 'pending', '{}'::jsonb,
         '{"model":"gpt-test","instructions":null,"input":"test","responseFormat":null}'::jsonb,
         'llm-user', 'llm-brand'
       )`,
    );
    ...
    await runtimePool.query("BEGIN");
    try {
      await runtimePool.query(`SET LOCAL ROLE ${contentRole}`);
      await runtimePool.query("SELECT set_config('venturecite.user_id', 'llm-user', true)");
      await runtimePool.query(
        `SELECT id FROM ${quotedPrivateSchema}.enqueue_openai_start_llm_job('llm-job-1')`,
      );
      await runtimePool.query("COMMIT");
    } catch (error) {
      await runtimePool.query("ROLLBACK").catch(() => undefined);
    ...
    const command = await pool.query<{
      ...
      `SELECT kind, length(payload_fingerprint)::int AS fingerprint_length, user_id, brand_id
       FROM ${quotedSchema}.outbox_commands
       WHERE idempotency_key = 'openai-start-llm-job:llm-job-1'`,
    );
    ...
    try {
      ...
      await expect(
        runtimePool.query(
          `UPDATE ${quotedSchema}.llm_jobs SET status = 'running' WHERE id = 'llm-job-1'`,
        ),
      ).rejects.toThrow();
    } finally {
      await runtimePool.query("ROLLBACK").catch(() => undefined);
    ...
    try {
      await runtimePool.query(`SET LOCAL ROLE ${workerRole}`);
      const linked = await runtimePool.query<{ status: string; response_id: string }>(
        `UPDATE ${quotedSchema}.llm_jobs
         SET status = 'running', response_id = 'response-1', started_at = now()
         WHERE id = 'llm-job-1'
         RETURNING status, response_id`,
      );
      expect(linked.rows).toEqual([{ status: "running", response_id: "response-1" }]);
      await runtimePool.query("COMMIT");
    } catch (error) {
      await runtimePool.query("ROLLBACK").catch(() => undefined);
      throw error;
  ...
  async function applyLlmJobOutboxMigration(): Promise<void> {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "migrations/0102_llm_job_outbox.sql"),
      "utf8",
    );
    const customized = source
      .replaceAll("public.", `${quotedSchema}.`)
