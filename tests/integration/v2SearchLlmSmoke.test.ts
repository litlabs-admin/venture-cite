...
if (databaseTest.kind === "ready") {
  ...
  describe("POST /search-llm persists data end-to-end", () => {
    beforeEach(async () => {
      await cleanup();
      await seed();
    });
    ...
    `);
      const facts = (
        factRows as unknown as { rows: Array<{ fact_key: string; fact_value: string }> }
      ).rows;
      expect(
    ...
    `);
      const logs = (
        logRows as unknown as {
          rows: Array<{ source: string; status: string; fact_count: number }>;
        }
      ).rows;
      expect(
  ...
  describe.skip("POST /search-llm persists data end-to-end", () => {
