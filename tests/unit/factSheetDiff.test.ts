...
const { storageMock } = vi.hoisted(() => ({
  storageMock: { getBrandFactSheetConflicts: vi.fn() },
}));
...
describe("GET /api/brand-fact-sheet/diff", () => {
  ...
  it("returns conflicts grouped by domain (client expects Record<Domain, ConflictPair[]>)", async () => {
    const pair = { userFact: { id: "u1", domain: "positioning" }, scrapedFact: { id: "s1" } };
    storageMock.getBrandFactSheetConflicts.mockResolvedValue([pair]);
    const res = await request(makeApp()).get("/api/brand-fact-sheet/diff?brandId=brand-1");
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true, conflicts: { positioning: [pair] } });
  });

  it("returns 400 when brandId is missing", async () => {
    const res = await request(makeApp()).get("/api/brand-fact-sheet/diff");
  ...
  it("returns 404 on cross-tenant brand", async () => {
    ...
    const res = await request(makeApp()).get("/api/brand-fact-sheet/diff?brandId=brand-other");
